...
class PlayState extends FlxState
{
	...
	override public function create():Void
	{
		var waterLevel = 151;
		var waveAmplitude = 5;
		var wavePeriod:Float = 10.0;
		var waveLength:Int = 100;
		var waveStep:Int = 4;
		// We need to offset camera position and make camera a bit bigger, so the final image won't be cut on both sides
		FlxG.camera.setPosition( -waveAmplitude, -waveAmplitude);
		FlxG.camera.setSize(FlxG.width + waveAmplitude * 2, FlxG.height + waveAmplitude * 2);
		var waterFilter = new WaterFilter(waterLevel, waveAmplitude, wavePeriod, waveLength, waveStep);
		FlxG.camera.setFilters([waterFilter]);
		...
		// We need to move map a bit, to take into account cameras' position offset due to "water" filter
		_collisionMap.setPosition(waveAmplitude, waveAmplitude); 
		
		// you can ignore all the rest of comments...
